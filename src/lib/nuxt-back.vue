...
<script>
  ...
  export default {
    props: ['type','top','right','bottom','size','color','duration','el_id'],
    data () {
      return {
        showIcon: false,
        scrollTop: 0,
    ...
    methods: {
      handelScroll () {
        let element= this.el_id?document.getElementById(this.el_id):document.documentElement
        let scrollTop=element.scrollTop
        scrollTop > this.defaultTop ? this.showIcon = true : this.showIcon = false
      },
      goTop () {
        scrollIt(0,this.defaultDuration,this.el_id,this.transitionName.default,this.handelScroll)
      }
    }
  }
</script>
